...
struct Args {
    /// Portal API base URL.
    #[arg(long, env = "PORTAL_API_URL", default_value = "http://localhost:3000/v1")]
...
async fn download_and_extract_demo(
    ...
    info!(size_bytes = compressed.len(), "Demo downloaded, decompressing bzip2");
...
async fn main() {
    ...
    loop {
        if let Err(e) =
            enrich_cycle(&portal, &mut gc, &args.game_slug, args.batch_size, args.skip_demo_rank)
                .await
        {
                ...
                let (player_ratings, map_name) = if !skip_demo_rank {
                    if let Some(ref url) = demo_url {
                        match download_and_extract_demo(&portal.http, url).await {
                            Ok(extraction) => {
                                let entries: Vec<PlayerRatingEntry> = extraction.ranks
                                    .into_iter()
