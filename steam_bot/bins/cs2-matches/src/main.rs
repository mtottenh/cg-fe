...
fn print_match(m: &MatchInfo, i: usize) {
    ...
    if !m.players.is_empty() {
        println!("  {:>12}  {:>3} {:>3} {:>3} {:>5} {:>2} {:>3}", "Account", "K", "A", "D", "Score", "HS", "MVP");
        println!("  {}", "─".repeat(42));
        for p in &m.players {
            println!(
                "  {:>12}  {:>3} {:>3} {:>3} {:>5} {:>2} {:>3}",
...
async fn run() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    let webapi_mode = args.api_key.is_some()
        && args.auth_code.is_some()
        && share_code.is_some();
    ...
    let matches = if let Some(sc) = share_code {
        ...
        let target = target.ok_or(
            "--target is required when CS2 hello fails (can't resolve own account ID)",
        )?;
...
async fn connect_gc(
    ...
    let profile = match cs2.hello().await {
        ...
        Err(e) => {
            if !args.json {
                println!("[!] CS2 hello failed ({}), continuing without profile.", e);
            }
