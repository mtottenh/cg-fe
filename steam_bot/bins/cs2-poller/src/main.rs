...
impl PortalClient {
    ...
    async fn get_active_tracking(
        &self,
        game: &str,
    ) -> Result<Vec<TrackingEntry>, reqwest::Error> {
        self.http
            .get(format!(
                "{}/internal/steam-tracking/active",
                self.base_url
            ))
    ...
    async fn submit_matches(
        &self,
        req: &SubmitMatchesRequest,
    ) -> Result<(), reqwest::Error> {
        self.http
            .post(format!(
                "{}/internal/discovered-matches",
                self.base_url
            ))
            .header("X-API-Key", &self.api_key)
            .json(req)
