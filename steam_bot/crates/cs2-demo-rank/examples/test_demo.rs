...
fn main() {
    ...
    let path = std::env::args()
        .nth(1)
        .expect("Usage: test_demo <path.dem or path.dem.bz2>");
    ...
    match cs2_demo_rank::extract_rank_updates(&demo_bytes) {
        Ok(ranks) => {
            ...
            for r in &ranks {
                println!(
                    "  steam_id64={} account_id={} rank_id={} rank_type_id={} wins={} rank_change={:.2}",
    ...
    // ---- Reference demoparser: per-player rank entity properties ----
    println!("\n=== Reference demoparser: per-player rank entity props ===");
    let huf = create_huffman_lookup_table();
    let settings = ParserInputs {
        wanted_events: vec!["rank_update".to_string()],
        wanted_player_props: RANK_PROPS.iter().map(|(_, prop)| prop.to_string()).collect(),
    ...
    println!("\nFound {} rank_update game events:", rank_events.len());
    for event in &rank_events {
        let get_str = |name: &str| -> Option<&str> {
            event.fields.iter().find(|f| f.name == name).and_then(|f| match &f.data {
        ...
        println!(
            "  steamid={:?} rank_new={:?} rank_type_id={:?} num_wins={:?} rank_change={:?}",
