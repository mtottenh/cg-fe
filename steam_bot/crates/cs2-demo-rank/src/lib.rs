...
impl<'a> ProtoCursor<'a> {
    ...
    fn skip_field(&mut self, wire_type: u32) -> Option<()> {
        match wire_type {
            0 => { self.read_varint64()?; }       // varint
            1 => { self.read_fixed64()?; }         // 64-bit
            2 => { self.read_length_delimited()?; } // length-delimited
            5 => { self.read_fixed32()?; }         // 32-bit
            _ => {}                                 // unknown, bail
        }
        Some(())
    }
        ...
        if msg_type == CS_UM_SERVER_RANK_UPDATE {
            *rank_msg_count += 1;
            if let Some(payload) = reader.read_bytes(size) {
                debug!(payload_len = payload.len(), "Found CS_UM_ServerRankUpdate message");
...
fn parse_rank_update_entry(buf: &[u8]) -> Option<RankUpdate> {
    ...
    while !cursor.is_eof() {
        ...
        match (field_number, wire_type) {
            ...
            (2, 0) => { cursor.read_int32(); } // rank_old — skip
...
mod tests {
    ...
    fn test_bit_reader_read_bytes_non_aligned() {
        ...
        let data = [0b11010_101u8, 0xAB, 0xCD, 0xEF];
        let mut reader = BitReader::new(&data);
