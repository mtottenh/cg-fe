...
impl MatchInfo {
    ...
    pub fn from_proto(m: &pb::CDataGCCStrike15_v2_MatchInfo) -> Self {
        ...
        let tv_port = m.watchablematchinfo.as_ref().map(|w| w.tv_port()).unwrap_or(0);

        let (team_scores, match_result, match_duration, players, share_parts) =
            match final_stats {
        ...
        let stats_map = final_stats
            .map(|s| s.map().to_string())
            .unwrap_or_default();
