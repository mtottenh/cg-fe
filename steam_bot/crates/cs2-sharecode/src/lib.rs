...
mod tests {
    ...
    const TEST_VECTORS: &[(&str, u64, u64, u16)] = &[
        ...
        ("CSGO-GADqf-jjyJ8-cSP2r-smZRo-TO2xK", 3230642215713767580, 3230647599455273103, 55788),
        ...
        ("CSGO-TKDTJ-YrAXs-sDNfL-HOuKO-i84VH", 3402250361329680757, 3402250801563828781, 61630),
        ("CSGO-p4X9o-3Mfut-tpe5y-J8K6f-mj5ZJ", 3402249502336221574, 3402252092201501292, 14119),
    ];

    #[test]
    fn decode_all_vectors() {
        for &(code, match_id, outcome_id, token) in TEST_VECTORS {
            let sc = ShareCode::decode(code).unwrap_or_else(|e| panic!("failed to decode {code}: {e}"));
    ...
    fn encode_all_vectors() {
        for &(code, match_id, outcome_id, token) in TEST_VECTORS {
            let sc = ShareCode { match_id, outcome_id, token };
