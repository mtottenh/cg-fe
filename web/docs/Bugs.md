...
- [ ] A 2-1 Bo3 cannot be submitted: game 3 has no map id (consequence of the decider bug; the page only feeds `picked`/`decider` maps, `useMatchDetail.ts:186-190`) and `isValidSubmission` refuses any played game without one, with no message on why Submit is disabled (`ResultSubmissionPanel.vue:393-411`).
- [ ] A 2-0 Bo3 is rejected on submit: the form posts every game row including the unplayed 0-0 with an empty `map_id`, and the server answers "Game scores don't sum to series score" (`ResultSubmissionPanel.vue:436-446`, `services/tournament/result.rs:819`). A two-row payload is accepted.
