...
- [x] The veto session is created, started and coin-flipped the moment the check-in window opens (15 min before kick-off), and the 30 s turn timeout then auto-bans before either captain has checked in (`portal-api/src/background/mod.rs:93, 494-510, 887-930`).
    - Fixed: (api) the pass now only pre-creates the session; the pick/ban hook starts it when both sides check in, a new pass step starts stranded sessions and flips unflipped coins after a 15 s grace (`PORTAL_VETO_COIN_FLIP_GRACE_SECS`), and timeouts skip matches still checking in. Integration test rewritten to pin the new order.
- [x] No action item or notification exists for a veto turn; the action-item query's status filter omits `pick_ban` (`portal-db/src/repositories/action_item.rs:80`).
    - Fixed: (api + web) `veto_turn` action item with the turn clock as its deadline, `pick_ban` added to the status filter; the bell shows 'Your turn in the map veto' and polls every 30 s instead of 60 s.
...
- [x] League page defaults to the newest-created season (the draft one) instead of the current one: picks `status === 'active'` else `seasons[0]`, and the list is `ORDER BY created_at DESC` (`useLeagueDetail.ts:85-94`, `portal-db/src/adapters/league_team/season.rs:177`). `leagues.current_season_id` exists with a trigger (migrations 0025:325, 0026:406) but is not exposed on `LeagueResponse`.
    - Fixed: (api + web) `current_season_id` is exposed on `LeagueResponse`; the page opens on it, else the first live season (active/playoffs/registration), else the latest completed one, never a draft.
- [x] League page's Tournaments section ignores the selected season (fetched by `league_id` only, `useLeagueDetail.ts:127-136`) while Teams/Awards/Stats obey it; `GET /v1/tournaments?season_id=` already works.
    - Fixed: tournaments are filtered to the selected season (tournaments tied to no season stay visible under every season).
- [x] Draft seasons appear in the public season selector for non-admin players, with no status label.
    - Fixed: the selector, the default and the `?season=` link only offer draft seasons to platform admins and league admins/moderators.
- [x] Creating a team on a draft season fails with "Tournament registration is closed": the season guard reuses `DomainError::RegistrationClosed` (`services/league_team/team.rs:160`), and the Create Team button is offered regardless of season status (`LeagueDetailPage.vue:224`).
    - Fixed: (api + web) `DomainError::LeagueSeasonNotOpen` → "This season isn't open for team registration yet"; Create Team is only offered while the selected season is in registration, with a chip explaining why otherwise.
- [x] Tournament registration never checks roster size against `tournament.team_size`: a one-player team registers for a 5-player cup and is marked Approved (`services/tournament/registration.rs:439-467`; client check `useTournamentContext.ts:90-105` also ignores roster).
    - Fixed: (api) `register_team` counts the active roster against `tournament.team_size` and refuses with "This team has N of the M players this tournament needs. Invite more players before registering."
- [x] Browse and detail routes (`/leagues`, `/leagues/:id`, `/tournaments`, `/tournaments/:slug`, `/players`) carry `requiresAuth` and bounce signed-out visitors to login, while every corresponding API read is public (`router/index.ts:40-72, 279-283`).
    - Fixed: `/leagues`, `/leagues/:id`, `/tournaments`, `/tournaments/:slug` no longer require sign-in; the header shows Leagues and Tournaments to guests and the tournament page offers "Sign in to register" with a redirect. `/players` stays members-only, which the router recorded as a community decision about profiles.
- [x] Map Veto Format on the tournament form is optional, clearable and has no default; a tournament created without one runs its matches with no veto at all, and the match stepper then shows Pick/Ban ticked as if it happened (`TournamentForm.vue:293-315`).
...
- [x] Admin dispute queue and detail modal show raw UUIDs for Match, Raised By, Original Winner and Result Claim, with no team names, tournament, or link to the match; the dispute DTO carries ids only (`AdminDisputesPage.vue:77-78, 192-196`, `DisputeDetailModal.vue:61-73`, `dto/responses/dispute.rs:19-45`).
    - Fixed: (api + web) disputes carry tournament and team names; the queue shows "A vs B" with the tournament and links to the match, and the modal shows names for Match, Raised By and Original Winner.
- [x] Starting a tournament manually leaves `starts_at` null, so cards read "Live Now" and "Start date TBD" at once (`TournamentCard.vue:62-68`); the admin tournament header and table also never show which league or season a tournament belongs to.
    - Fixed: (api + web) start sets `starts_at` if unset; the admin tournaments table gained a League column and the admin tournament header shows "League · Season".
- [x] Season dates (`registration_start/end`, `season_start/end`) exist in the schema and the DTO but nothing sets or displays them: the admin seasons panel shows "-" under Registration and the public page has no dates (migration 0025:34-37, `dto/responses/league_team.rs:32-38`).
    - Fixed: the season create and edit modals take registration and season dates; the seasons panel shows them (or a 'No dates' warning) and the public league page prints "Registration … · Plays …" under the season selector.
